...
sulfate =  pm.AerosolSpecies('sulfate',
                             ...
                             kappa=0.54, bins=200)
Anthropogenic_OC = pm.AerosolSpecies('sea salt',
                             pm.Lognorm(mu=0.85, sigma=1.2, N=10.),
                             kappa=1.2, bins=40)
#Anthropogenic_OC= pm.AerosolSpecies('OC',pm.Lognorm(mu=0.12/2,sigma=1.6,N=400.),kappa=0.2,bins=100)
...
initial_aerosols = [sulfate, Anthropogenic_OC]
...
ss = axA.plot(sulf_array[:, ::10]*1e6, parcel_trace['z'], color=sul_c,
         label="sulfate")
sa = axA.plot(sea_array*1e6, parcel_trace['z'], color=sea_c, label="OC")
axA.set_xscale('log')
axA.set_xlim(1e-2, 10.)
axA.set_xticks([1e-2, 1e-1, 1e0, 1e1], [0.01, 0.1, 1.0, 10.0])
axA.legend([ss[0], sa[0]], ['sulfate', 'sea_salt'], loc='upper right')
